...
func createShoppingCartService() *http.Server {
	...
	cartMux.Handle("/api/carts", &validationMiddleware{next: http.HandlerFunc(cartsHandler)})
...
func (vm validationMiddleware) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	res, err := http.Head(fmt.Sprintf("http://localhost:3000/api/customers/%v", c.CustomerID))
	if err != nil {
		log.Print(err)
		w.WriteHeader(http.StatusInternalServerError)
		return
