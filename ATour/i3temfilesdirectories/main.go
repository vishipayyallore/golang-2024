...
func main() {
	/*
	 */
	header.DisplayHeader("Showing Temporary Files and Directories")
	...
	// Display the name of the temporary file. On Unix-based OSes the directory will likely be /tmp. The file name starts with the
	// prefix given as the second argument to os.CreateTemp and the rest is chosen automatically to ensure that concurrent calls will
	// always create different file names.
	utl.PLine("Temp file name: ", f.Name())

	// Clean up the file after we’re done. The OS is likely to clean up temporary files by itself after some time, but it’s good practice
