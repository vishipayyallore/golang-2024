...
func main() {
	...
	header.DisplayHeader("Showing Closing Channels")
	...
	go func() {
		for {
			j, more := <-jobs
			if more {
				utl.PLine("Received job ", j)
			} else {
				utl.PLine("Received all jobs")
				done <- true
				return
			}
		}
	...
	for j := 1; j <= 3; j++ {
		jobs <- j
		utl.PLine("Sent job ", j)
	}
	close(jobs)
	utl.PLine("Sent all jobs\n")
	...
	_, ok := <-jobs
	utl.PLine("\nJobs channel is open? ", ok)
}

/*
Notes:

	// Our running program shows that the worker received all the jobs. It’s idiomatic to use
	// close when the sender knows that no more values will be sent on a channel.
	// This is a useful feature to avoid a deadlock in the receiver side.
